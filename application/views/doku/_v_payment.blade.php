    ...
    <section class="default-width"><!-- start content -->
        ...
        <div class="head padd-default"><!-- start head -->
            ...
            <div class="right-head fright">
                ...
                <div class="amount color-one">{{ number_format($_POST['amount'], 2, ',', '.') }}</div>
            </div>
            <div class="clear"></div>
        </div><!-- end head -->
        ...
        <div class="content-payment-channel padd-default"><!-- start content payment channel -->
            <form method="post" action="{{ $endPointURL.'/mandiri_click_pay' }}">
                <div id="mandiriclickpay" class="channel"> <!-- mandiri clickpay -->
                    ...
                    <div class="list-chacode">
                        <ul>
                            ...
                            <li>
                                ...
                                <div class="num-chacode">{{ number_format($_POST['amount'], 0, '', '') }}</div>
                                <input type="hidden" name="CHALLENGE_CODE_2" value="{{ number_format($_POST['amount'], 0, '', '') }}"/>
                                <div class="clear"></div>
                            </li>
                            <li>
                    ...
                    <div class="validasi">
                        <div class="styled-input fleft width50">
                            ...
                            <input type="hidden" name="invoice" value="{{ $_POST['trans_id'] }}">
                            <input type="hidden" name="amount" value="{{ $_POST['amount'] }}">
                            <input type="hidden" name="mallid" value="{{ $_POST['mall_id'] }}">
                            <input type="hidden" name="chain" value="{{ $_POST['chain_merchant'] }}">
...
@else
    ...
    @php
        if ($paymentchannel=="15"){
            $serverurl = $endPointURL.'/credit_card';
        } else if ($paymentchannel=="04"){
            $serverurl = $endPointURL.'/doku_wallet';
        } else {
            $serverurl = $endPointURL.'/doku';
        }
    @endphp
    ...
    <head>
        ...
        <script src="https://staging.doku.com/doku-js/assets/js/doku.js?version={{ time() }}"></script> <!-- To prevent js caching -->
        ...
        <script type="text/javascript">
            $(function() {
                ...
                data.req_merchant_code      = '{{ $_POST['mall_id'] }}'; //mall id or merchant id
                data.req_chain_merchant     = '{{ $_POST['chain_merchant'] }}'; //chain merchant id
                data.req_payment_channel    = '{{ $_POST['payment_channel'] }}'; //payment channel
                data.req_server_url         = '{{ $serverurl }}'; //merchant payment url to receive pairing code & token
                data.req_transaction_id     = '{{ $_POST['trans_id'] }}'; //invoice no
                data.req_amount             = '{{ $_POST['amount'] }}';
                data.req_currency           = '{{ $_POST['currency'] }}'; //360 for IDR
                data.req_words              = '{{ $words }}'; //your merchant unique key
                data.req_session_id         = new Date().getTime(); //your server timestamp
                data.req_form_type          = '{{ @$_POST['form_type'] }}';
