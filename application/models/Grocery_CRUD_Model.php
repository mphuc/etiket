...
class Grocery_CRUD_Model  extends CI_Model  {
    ...
    function get_list()
    {
        ...
        {
            $select = $this->relation_n_n_queries($select,$select_list=1);
            $results = $select;
        }else{
    ...
    protected function relation_n_n_queries($select, $select_list = null)
    {
        ...
        foreach($this->relation_n_n as $relation_n_n)
        {
            list($field_name, $relation_table, $selection_table, $primary_key_alias_to_this_table,
                        $primary_key_alias_to_selection_table, $title_field_selection_table, $priority_field_relation_table, $where_clause, $having_clause) = array_values((array)$relation_n_n);
            ...
            if($relation_table == 'tb_terms') {
                $type = str_replace("tb_","",$relation_table).'_type';
                $andwhere = "$type='$field_name' and";
            }else {
                $andwhere ="";
            }
            ...
            $select .= ", (SELECT GROUP_CONCAT(DISTINCT $field) FROM $selection_table "
                ...
                ."WHERE $andwhere $relation_table.$primary_key_alias_to_this_table = `{$this->table_name}`.$this_table_primary_key GROUP BY $relation_table.$primary_key_alias_to_this_table) AS $field_name";
        }
    ...
    function get_relation_n_n_selection_array($primary_key_value, $field_info)
    {
        ...
        //Rombak Grocery Edit CRUD
        if($field_info->relation_table == 'tb_terms') {
            $type = str_replace("tb_","",$field_info->relation_table);;
            $this->db->where($type.'_type', $field_info->field_name);
        }
        $this->db->join(
            ...
            foreach($post_data as $primary_key_value)
            {
                ...
                if($field_info->relation_table == 'tb_terms') {
                    $where_array = array(
                        ...
                        $type.'_type' => $field_info->field_name,
                    );
                }else {
                     $where_array = array(
                        $field_info->primary_key_alias_to_this_table => $main_primary_key,
    ...
    {
        $primary_key_field = $this->get_primary_key();
        return $this->db->update($this->table_name,$this->security->xss_clean($post_array), array( $primary_key_field => $primary_key_value));
    }

    function db_insert($post_array)
    {
        $insert = $this->db->insert($this->table_name,$this->security->xss_clean($post_array));
        if($insert)
        {
