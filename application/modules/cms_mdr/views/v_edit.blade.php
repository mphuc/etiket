    ...
    <div id="page_content" ng-controller="editController">
        ...
        <div id="page_content_inner" style="display: none;">
            <form id="form" action="" method="post" class="form-div uk-form-stacked" enctype="multipart/form-data" accept-charset="utf-8" onsubmit="return false;">
                <input type="hidden" name="mdr_id" value="{{ $row[$pk] }}">
                                ...
                                <div class="uk-grid uk-grid-divider uk-grid-medium" data-uk-grid-margin>
                                    <div class="uk-width-large-1">
                                        ...
                                        {{-- <div class="uk-form-row">
                                            ...
                                            <input class="md-input" id="field-bank_name" name="bank_name" type="text" value="{{$row["bank_name"]}}" maxlength="255" >
                                        </div>
                                        <div class="uk-form-row">
                                            <label class="uk-form-label" for="field-bank_order">Order</label>
                                            <input class="md-input" id="field-bank_order" name="bank_order" type="number" value="{{$row["bank_order"]}}" />
                                        </div> --}}
                                        ...
                                        <div class="uk-form-row">
                                            ...
                                            <select class="md-input" name="id_bank" id="id_bank" style="margin-top: 20px;" onclick="cekBank()">
                                                ...
                                                @foreach ($bank as $item)
                                                    <option value="{{ $item->bank_id }}" nama="{{ $item->bank_name }}" {{ $item->bank_id == $row['id_bank'] ? 'selected' : '' }}>{{ $item->bank_name }}</option>
                                                @endforeach
                                            </select>
                                            <input type="hidden" name="mdr_bank" id="mdr_bank" value="{{ $row['mdr_bank'] }}" readonly> 
                                        </div>
                                        <div class="uk-form-row">
                                            ...
                                            <select class="md-input" name="id_card_type" id="id_card_type" style="margin-top: 20px;" onclick="cekCard()">
                                                ...
                                                @foreach ($type_card as $item)
                                                    <option value="{{ $item->card_type_id }}" nama="{{ $item->card_type_name }}" {{ $item->card_type_id == $row['id_card_type'] ? 'selected' : '' }}>{{ $item->card_type_name }}</option>
                                                @endforeach
                                            </select>
                                            <input type="hidden" name="mdr_card_type" id="mdr_card_type" value="{{ $row['mdr_card_type'] }}" readonly>                                            
                                        </div>
                                        <div class="uk-form-row">
                                            <label class="uk-form-label" for="field-bank_order">Value MDR</label>
                                            <input type="text" class="md-input" name="mdr_value" id="mdr_value" value="{{ $row['mdr_value'] }}" maxlength="4">
