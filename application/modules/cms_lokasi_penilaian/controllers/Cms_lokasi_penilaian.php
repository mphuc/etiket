 ...
 */
class Cms_lokasi_penilaian extends CI_Controller {
    ...
    public function index()
    {
       try {
            ...
            $umbrella   = $this->base_config->groups_access('lokasi_penilaian');

            $umbrella->set_theme('twitter-bootstrap')
                     ->set_table('tb_location_rate')
                     ->set_subject('Rating Location')
                     ->columns('location_rate_name','location','check_location')
                     ->order_by('location_rate_id','desc')
                     ->unset_texteditor('location','full_text')
                     ->unset_export()
                     ->unset_print()
                     ->add_fields('location_rate_name','location')
                     ->edit_fields('location_rate_name','location')
    ...
    */
    ...
    {
        return '<input type="checkbox" class="removelist" value="' . $row->location_rate_id . '" name="' . $row->location_rate_id . '" id="' . $row->location_rate_id . '" data-md-icheck />';
    }
}