    ...
    <div id="page_content" ng-controller="editController">
        ...
        <div id="page_content_inner" style="display: none;">
            <form id="form" action="" method="post" class="form-div uk-form-stacked" enctype="multipart/form-data" accept-charset="utf-8" onsubmit="return false;">
                <input type="hidden" name="card_type_id" value="{{ $row[$pk] }}">
                                ...
                                <div class="uk-grid uk-grid-divider uk-grid-medium" data-uk-grid-margin>
                                    <div class="uk-width-large-1">
                                        ...
                                        <div class="uk-form-row">
                                            <label class="uk-form-label" for="field-card_type_name">Tipe Kartu</label>
                                            <input class="md-input" id="field-card_type_name" name="card_type_name" type="text" value="{{$row["card_type_name"]}}" maxlength="255" >
                                        </div>
                                        <div class="uk-form-row">
                                            <label class="uk-form-label" for="field-card_type_order">Order</label>
                                            <input class="md-input" id="field-card_type_order" name="card_type_order" type="number" value="{{$row["card_type_order"]}}" />
                                        </div>
                                        <div class="uk-form-row">
                                            <label class="uk-form-label" for="field-card_type_order">Type Card</label>
                                            <select class="md-input" name="id_payee" id="id_payee" style="margin-top: 20px;">
                                                <option value="">-- Type Card --</option>
                                                @foreach ($payee as $item)
                                                    <option value="{{ $item->payee_id }}" nama="{{ $item->payee_name }}" {{ $item->payee_id == $row['id_payee'] ? 'selected' : '' }}>{{ $item->payee_name }}</option>
                                                @endforeach
                                            </select>                                           
                                        </div>
                                                                                                                                                                                                                                                                                    </div>
