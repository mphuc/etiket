 ...
 */
class Cms_users extends MX_Controller
{
    ...
    public function add()
    {
        ...
        if( $_POST ){
            ...
            $this->form_validation->set_rules('email', 'Email', 'required|valid_email|is_unique[tb_user.email]|check_email');
            $this->form_validation->set_rules('user_mobile', 'Mobile', 'is_unique[tb_user.user_mobile]');
            $this->form_validation->set_rules('password', 'Kata Sandi', 'required');

            $this->form_validation->set_message('check_email', 'Bidang email harus unik dan wajib diisi.');
        ...
        $data['pk'] = $this->get_primary_key()->name;
        $data['cabang'] = "STUDIO ALAM TV9";
        $allGroup = $this->db->get('tb_groups')->result_array();
    ...
    public function edit()
    {
        ...
        $data['row'] = $results;
        $data['pk'] = $this->get_primary_key()->name;
        $data['cabang'] = "Benteng Vrederbug";
        $allGroup = $this->db->get('tb_groups')->result_array();
        $data['groups'] = $allGroup;
        $this->db->where('user_id',$id);
        $userGroups = $this->db->get('tb_users_groups')->result_array();
