    ...
    <div id="page_content" ng-controller="editController">
        ...
        <div id="page_content_inner" style="display: none;">
            <form id="form" action="" method="post" class="form-div uk-form-stacked" enctype="multipart/form-data" accept-charset="utf-8" onsubmit="return false;">
                <input type="hidden" name="mp_id" value="{{ $row[$pk] }}">
                                ...
                                <div class="uk-grid uk-grid-divider uk-grid-medium" data-uk-grid-margin>
                                    <div class="uk-width-large-1">
                                        ...
                                        <div class="uk-form-row">
                                            <label class="uk-form-label" for="field-product_package_id">Package</label>
                                            <select id="field-product_package_id" name="product_package_id" class="md-input" data-md-selectize>
                                                @foreach($disc as $v)
                                                    <option value="{{$v['dp_id']}}" @if($row['product_package_id'] == $v['dp_id']) selected @endif>{{$v['dp_name']}}</option>
                                                @endforeach
                                            </select>
                                        </div>

                                        <div class="uk-form-row">
                                            <label class="uk-form-label" for="field-product_combination_id">Product</label>
                                            <select id="field-product_combination_id" name="product_combination_id" class="md-input" data-md-selectize>
                                                @foreach($products as $v)
                                                    <option value="{{$v['product_id']}}" @if($row['product_combination_id'] == $v['product_id']) selected @endif>{{$v['product_title']}}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                                                                                                                                                                                                                                                                                                                                                                
                                        <div class="uk-form-row">
                                            <label class="uk-form-label" for="field-mp_discount">Discount (%)</label>
                                            <input class="md-input" id="field-mp_discount" name="mp_discount" type="number" value="{{$row["mp_discount"]}}" maxlength="11" />
                                        </div>
                                                                                                                                                                                                                                            </div>
                                </div>
                            </div>
