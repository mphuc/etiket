 ...
 */
class Cms_discount_product extends MX_Controller
{
    protected $table = 'tb_discount_product';
    protected $subject = 'Data discount_product';
    protected $module;
    protected $primary_key = 'dp_id';
    ...
    public function detail()
    {
        ...
        $this->db->select('p.product_title,c.*');
        $this->db->join('tb_product p','p.product_id=c.product_combination_id');
        $this->db->where('product_package_id', $id);
        $data = $this->db->get('tb_member_card_package c')->result_array();
        json_response($data);
    }
