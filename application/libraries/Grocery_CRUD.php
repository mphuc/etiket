...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function showList($ajax = false, $state_info = null)
	{
		...
		$data->state_list_url		= $this->getStateListUrl();
	...
	protected function get_integer_input($field_info,$value)
	{
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$value' class='numeric md-input' $extra_attributes />";
		return $input;
	}
	...
	protected function get_true_false_input($field_info,$value)
	{
		...
		$input .= "<span class='icheck-inline'><input id='field-{$field_info->name}-true' data-md-icheck type='radio' name='{$field_info->name}'  value='1' $checked /><label class='inline-label'> ".$true_string."</label> </span>";
		...
		$input .= "<span class='icheck-inline'><input id='field-{$field_info->name}-false' data-md-icheck type='radio' name='{$field_info->name}' value='0' $checked /> <label class='inline-label'>".$false_string."</label></span>";
	...
	protected function get_string_input($field_info,$value)
	{
		...
		if(!empty($field_info->db_max_length))
			$extra_attributes .= "maxlength='{$field_info->db_max_length}'";
		$input = "<input class='md-input' id='field-{$field_info->name}' name='{$field_info->name}' type='text' value=\"$value\" $extra_attributes />";
		return $input;
	}
	...
	protected function get_text_input($field_info,$value)
	{
		if($field_info->extras == 'text_editor')
		{
			...
			$input = "<textarea id='field-{$field_info->name}' name='{$field_info->name}'  >$value</textarea><script>if(CKEDITOR.instances['field-{$field_info->name}'])delete CKEDITOR.instances['field-{$field_info->name}'];CKEDITOR.replace( 'field-{$field_info->name}' ,{filebrowserBrowseUrl : '".base_url()."assets/filemanager/dialog.php?type=2&editor=ckeditor&fldr=&akey=dsflFWR9u2xQa',filebrowserUploadUrl : '".base_url()."assets/filemanager/dialog.php?type=2&editor=ckeditor&fldr=&akey=dsflFWR9u2xQa',filebrowserImageBrowseUrl : '".base_url()."assets/filemanager/dialog.php?type=1&editor=ckeditor&fldr=&akey=dsflFWR9u2xQa'});</script>";
		}
		else
		{
			$input = "<br><textarea cols='30' rows='3' class='md-input' data-autosize-on='true' style='overflow: hidden; word-wrap: break-word; height: 90px;' id='field-{$field_info->name}' name='{$field_info->name}'>$value</textarea>";
		}
		return $input;
	}
	...
	protected function get_datetime_input($field_info,$value)
	{
		...
		$input = "<br><input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$datetime'  maxlength='19' class='timepic' />";
		return $input;
	}
	...
	protected function get_password_input($field_info,$value)
	{
		...
		if(!empty($field_info->db_max_length))
			$extra_attributes .= "maxlength='{$field_info->db_max_length}'";
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='password' class='md-input' value='$value' $extra_attributes />";
		return $input;
	}
	...
	protected function get_date_input($field_info,$value)
	{
		...
		$format="";
		$input = "<br><input id='field-{$field_info->name}'  class='datepic' name='{$field_info->name}' type='text' value='$date' maxlength='10' class='md-input' />
	...
	protected function get_dropdown_input($field_info,$value)
	{
		...
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}' data-md-selectize data-placeholder='".$select_title."'>";
	...
	protected function get_set_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<br><select id='field-{$field_info->name}' name='{$field_info->name}[]' multiple='multiple' size='8' class='chosen-multiple-select' data-placeholder='$select_title' style='width:510px;' >";
	...
	protected function get_relation_n_n_input($field_info_type, $selected_values)
	{
		...
		if(empty($unselected_values) && empty($selected_values))
		{
			...
			if($field_info_type->name != 'groups' && $field_info_type->name !='role' && $field_info_type->name != 'category')
				$input .="<button data-uk-modal=\"{target:'.modal{$field_info_type->name}'}\" type='button' id='{$field_info_type->name}' class='md-btn md-btn-primary'>".$this->l('add_new')."</button>";
				$input .='
					<div class="uk-modal modal'.$field_info_type->name.'">
	                                <div class="uk-modal-dialog">
	                                    <div class="uk-modal-header">
	                                        <h3 class="uk-modal-title">'.$this->l('list_add').' '.$field_info_type->display_as.'</h3>
	                                    </div>
	                                    <p><div class="uk-form-row"><label>'.$field_info_type->display_as.'</label><input type="text" class="md-input input'.$field_info_type->name.'"  /></div></p>
	                                    <div class="uk-modal-footer uk-text-right">
	                                        <button type="button" class="md-btn md-btn-flat uk-modal-close">'.$this->l('close').'</button><button type="button" onclick="savemultiple(\''.$field_info_type->name.'\')" class="md-btn md-btn-flat md-btn-flat-primary">'.$this->l('list_add').' '.$field_info_type->display_as.'</button>
	                                    </div>
	                                </div>
	                            </div>';
		}
		else
		{
			...
			$input = "<br><select id='field-{$field_info_type->name}' name='{$field_info_type->name}[]' multiple='multiple' size='8' class='$css_class field-{$field_info_type->name}' data-placeholder='$select_title' style='$width_style' >";
	...
	protected function get_upload_file_input($field_info, $value)
	{
		...
		$input = '<span class="uk-form-file md-btn md-btn-primary" id="upload-button-'.$unique.'" style="'.$uploader_display_none.'">
		...
		$input .= $is_image ? " $image_class'><div class='uk-margin-bottom uk-text-center uk-position-relative'><img class='img_medium' src='".$file_url."' width='100px'>" : "' target='_blank'>$value";
		$input .= "</a> ";
		$input .= "<br><a href='javascript:void(0)' id='delete_$unique' class='md-btn md-btn-danger'>".$this->l('form_upload_delete')."</a> </div>";
		$input .= "<div class='uk-modal' id='modalimg_$unique'><div class='uk-modal-dialog uk-modal-dialog-lightbox'><button type='button' class='uk-modal-close uk-close uk-close-alt'></button><img id='modalimgbig_$unique' src='".$file_url."' alt=''/></div></div><div style='clear:both'></div>";
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
	...
	protected function getStateListUrl()
	{
		$ci = &get_instance();
		$class= $ci->uri->segment(2);
		return base_url().'cms/'.$class.'/index';
	}
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	protected $list_upload 			= null;
	protected $lisfilter 			= null;
	protected $listfilterfeed 		= null;
	protected $bulkaction 			= null;
	protected $actionfalse 			= null;
...
//@todo ui_day, ui_month, ui_year has to be lang strings
	...
	protected function get_common_data()
	{
		...
		$data->actionfalse				= $this->actionfalse;

		$data->list_upload			= $this->list_upload;
		$data->lisfilter			= $this->lisfilter;
		$data->listfilterfeed		= $this->listfilterfeed;
	 ...
	 * @return grocery_CRUD
	 */
	public function set_composer( $subject )
	{
	 ...
	 * @return grocery_CRUD
	 */
	public function set_actionfalse( $subject )
	{
		$this->actionfalse 			= $subject;
	 ...
	 * @return grocery_CRUD
	 */
	public function set_bulkactionfalse( $subject )
	{
		$this->bulkaction 			= $subject;
	 ...
	 * @return grocery_CRUD
	 */
	public function set_listfilter( $subject )
	{
		$this->lisfilter 			= $subject;
	 ...
	 * @return grocery_CRUD
	 */
	public function set_listfilterfeed( $subject )
	{
		$this->listfilterfeed 			= $subject;
	 ...
	 * @return grocery_CRUD
	 */
	public function set_list_upload( $subject )
	{
		$this->list_upload 			= $subject;
	 ...
	 * @param mixed $where_clause
	 */
	public function set_relation_n_n($field_name, $relation_table, $selection_table, $primary_key_alias_to_this_table, $primary_key_alias_to_selection_table , $title_field_selection_table , $priority_field_relation_table = null, $where_clause = null,$having_clause = null)
	{
