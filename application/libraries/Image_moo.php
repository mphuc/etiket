...
class Image_moo
{
	...
	//----------------------------------------------------------------------------------------------------------
	{
		...
		// ok, lets go!
		if ($filename=="") $filename=rand(1000,999999).".png";					// send as jpeg
	...
	//----------------------------------------------------------------------------------------------------------
	{
		if (!is_resource($this->temp_image))
		{
			...
			imagefilledrectangle($this->temp_image, 0, 0, $this->width, $this->height, imagecolorallocate($this->temp_image, 255, 255, 255));
			// check it
			if(!is_resource($this->temp_image))
			{
	...
	//----------------------------------------------------------------------------------------------------------
	{
		...
		$this->_copy_to_temp_if_needed();
		for ($i = 0; $i < 60; ++$i) {
		    imagefilter($this->temp_image, IMG_FILTER_GAUSSIAN_BLUR);
		} 
		// set sizes
		$this->_set_new_size();
		// return self
