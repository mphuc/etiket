...
class Plugins {
    ...
    */
    public function get_activated_plugins()
    {
        ...
        $plugins = $this->_ci->db->get('tb_plugins');
    ...
    public function include_plugins()
    {
        if(self::$plugins_active AND !empty(self::$plugins_active))
        {
            ...
            foreach (self::$plugins_active AS $name => $value)
            {
                ...
                $this->_ci->db->where('plugin_system_name', $name)->update('tb_plugins', $data);
    ...
    */
    public function activate_plugin($name)
    {
        ...
        {            
            $db = $this->_ci->db->select('plugin_system_name')->where('plugin_system_name', $name)->get('tb_plugins', 1);
            
            if ($db->num_rows() == 0)
            {
                $this->_ci->db->insert('tb_plugins', array('plugin_system_name' => $name));
            }
    ...
    */
    public function deactivate_plugin($name)
    {
        ...
        {
            $this->_ci->db->where('plugin_system_name', $name)->delete('tb_plugins');
